...
router.post('/:image',passport.authenticate('jwt', { session: false }),  (req, res) => {
  upload(req, res, (err) => {
    if (err) {
      res.status(500).json(err);
    } else if (req.params.image === 'gallery') {
      const filename = { image: req.file.filename };
      connection.query('INSERT INTO gallery SET ?', filename, (errTwo, results) => {
        if (errTwo) {
          res.status(500).send("Erreur lors de l'ajout de l'image en base de données");
          console.log(errTwo.message);
        } else {
          connection.query('SELECT * FROM gallery WHERE id= ?', results.insertId, (errThree, pic) => {
            if (errThree) {
